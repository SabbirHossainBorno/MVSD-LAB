...
export default function Sidebar({ isOpen, toggleSidebar }) {
  ...
  const handleClickOutside = useCallback((event) => {
    if (event.target.closest('aside') === null && isOpen) {
      toggleSidebar();
    }
  ...
  return (
    <aside className={`fixed top-0 left-0 w-64 bg-[rgb(17,24,39)] text-white h-full p-6 transition-transform transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative md:w-64 md:flex md:flex-col z-50 shadow-lg`}>
      {/* Close Button */}
      <button
