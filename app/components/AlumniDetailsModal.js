...
import { FiX, FiBriefcase, FiAward, FiGlobe, FiCalendar, FiInfo } from 'react-icons/fi';
            ...
            <div className="flex border-b border-gray-700 mb-6">
              {['basic', 'education', 'career', 'social'].map((tab) => (
                <button
                  ...
                  className={`px-6 py-3 flex items-center gap-2 capitalize ${
                    activeTab === tab
                ...
                >
                  {tab === 'basic' && <FiInfo />}
                  {tab === 'education' && <FiAward />}
                  {tab === 'career' && <FiBriefcase />}
                  {tab === 'social' && <FiGlobe />}
                  {tab}
                </button>
              ))}
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <DetailItem
                      ...
                      value={format(new Date(alumni.completion_date), 'd MMM yyyy')}
                    />
                    <DetailItem label="Passport Number" value={alumni.passport_number} />
                    <DetailItem label="ID Number" value={alumni.id_number} />
                  </div>
                </div>
              )}
              ...
              {activeTab === 'education' && (
                <div className="space-y-4">
                  {alumni.education?.filter(e => e.degree).map((edu, index) => (
              ...
              {activeTab === 'career' && (
                <div className="space-y-4">
                  {alumni.career?.filter(c => c.position).map((job, index) => (
                    <div key={index} className="p-4 bg-gray-700/30 rounded-lg">
              ...
              {activeTab === 'social' && (
                <div className="space-y-3">
                  {alumni.social_media?.filter(s => s.link).map((social, index) => (
                    <a
                      key={index}
                      href={social.link}
                      target="_blank"
