// app/dashboard/director_add/page.js
'use client';
...
const AddDirector = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (age < 18) {
        toast.error('Director must be at least 18 years old');
        return;
      }

  
      // ID validation 
      if (/^0/.test(formData.bannerID)) {
        toast.error('Banner ID cannot start with 0');
        return;
      }
      ...
      const response = await fetch('/api/director_add', {
        method: 'POST',
        body: data,
      ...
      if (response.ok) {
        toast.success(`Director Added Successfully! ID: ${result.directorId}`);
          ...
          >
            {loading ? (
              <span className="flex items-center justify-center">
                ...
                <span className="pt-[2px]">Adding Director....</span>
              </span>
            ) : (
              'Add Director'
            )}
...
export default withAuth(AddDirector, 'admin'); // Pass 'admin' as the required role