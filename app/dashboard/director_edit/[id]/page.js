// app/dashboard/director_edit/[id]/page.js
'use client';
...
const EditDirector = () => {
  ...
  useEffect(() => {
    const fetchDirectorData = async () => {
      try {
        const response = await fetch(`/api/director_edit/${id}`);
        ...
        toast.error('Failed to fetch director data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (section) => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to update director details', {
        autoClose: 5000,
        hideProgressBar: false,
      });
    } finally {
...
export default withAuth(EditDirector, 'admin'); // Pass 'admin' as the required role