// app/dashboard/member_list/staff_member_list/page.js
'use client';
...
const StaffMembersList = () => {
  ...
  const [totalStaffMembers, setTotalStaffMembers] = useState(0);
  const [activeStaffMembers, setActiveStaffMembers] = useState(0);
  const [inactiveStaffMembers, setInactiveStaffMembers] = useState(0);
  const [graduateStaffMembers, setGraduateStaffMembers] = useState(0);
  ...
  useEffect(() => {
    const fetchStaffMembers = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setInactiveStaffMembers(data.inactiveStaffMembers ?? 0);
          setGraduateStaffMembers(data.graduateStaffMembers ?? 0);
        } else {
    ...
    const debounceFetch = setTimeout(fetchStaffMembers, 300); // Debounce for 300ms
...
export default withAuth(StaffMembersList, 'admin'); // Pass 'admin' as the required role