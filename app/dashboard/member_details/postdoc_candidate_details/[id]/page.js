// app/dashboard/member_details/postdoc_candidate_details/[id]/page.js
'use client';
...
const PostdocCandidateDetails = () => {
  const [postdocCandidateDetails, setPostdocCandidateDetails] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchPostdocCandidateDetails = async () => {
      try {
        const response = await fetch(`/api/member_details/postdoc_candidate_details/${id}`, { signal: controller.signal });
        if (!response.ok) throw new Error('Network error');
        const data = await response.json();
        setPostdocCandidateDetails(data);
      } catch (error) {
        if (error.name !== 'AbortError') console.error('Failed to fetch Post Doc candidate details:', error);
      } finally {
        setLoading(false);
      }
    };
  
    fetchPostdocCandidateDetails();
  ...
  if (!postdocCandidateDetails || !postdocCandidateDetails.basicInfo) return <div>Post Doc Candidate Details Not Found</div>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-blue-900 text-slate-100 p-4 sm:p-8">
      <div className="max-w-7xl mx-auto space-y-4">
          ...
          <div className="flex flex-col items-center space-y-6 group">
              ...
              <div className="relative w-48 h-48 shrink-0 transition-transform duration-500 hover:scale-[1.02] hover:rotate-[1deg]">
                ...
                <div className="relative overflow-hidden rounded w-full h-full shadow-2xl shadow-blue-300/30 hover:shadow-purple-400/40 transition-all duration-500">
                  <Image
                    src={formatImageUrl(postdocCandidateDetails.basicInfo.photo)}
                    alt={`${postdocCandidateDetails.basicInfo.first_name} ${postdocCandidateDetails.basicInfo.last_name}`}
                    width={192}
                    height={192}
              ...
              <div className="relative inline-block">
                <div className="relative px-6 py-2 bg-white/5 backdrop-blur-sm border-2 border-blue-400/20 rounded shadow-lg hover:border-purple-400/40 transition-colors duration-300">
                  <h3 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent 
                    ...
                    {postdocCandidateDetails.basicInfo.id}
                  </h3>
                </div>
                {/* Status Indicator */}
                <div className={`absolute -top-2.5 -right-2.5 flex items-center gap-1.5 px-3 py-0.5 rounded text-xs font-semibold backdrop-blur-lg border ${
                  postdocCandidateDetails.basicInfo.status === 'Active' 
                  ...
                  {postdocCandidateDetails.basicInfo.status}
                  </div>
              </div>
            </div>
            <div className="text-center md:text-left space-y-4">
              <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">
                {`${postdocCandidateDetails.basicInfo.first_name} ${postdocCandidateDetails.basicInfo.last_name}`}
              </h1>
              <p className="text-xl text-gray-300 font-light max-w-2xl text-justify">
                {postdocCandidateDetails.basicInfo.short_bio || 'Dedicated Post Doc Candidate at MVSD Lab'}
              </p>

              <div className="flex justify-center md:justify-start space-x-4">
                {postdocCandidateDetails.socialMedia.map((social, index) => (
                  <a
                    key={index}
                    href={social.link}
                    target="_blank"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="bg-gray-800/50 backdrop-blur-lg p-8 rounded shadow-xl">
            ...
            <div className="space-y-4">
              <InfoItem icon={<FiMail />} label="Email" value={postdocCandidateDetails.basicInfo.email} />
              <InfoItem
                icon={<FiMail />}
                label="Other Email"
                value={
                  Array.isArray(postdocCandidateDetails.basicInfo.other_emails) && 
                  postdocCandidateDetails.basicInfo.other_emails.length > 0 ? (
                    <div className="flex flex-wrap items-center gap-x-2">
                      {postdocCandidateDetails.basicInfo.other_emails.map((email, index) => (
                        <span key={index} className="flex items-center">
                          <span className="break-words">{email}</span>
                          {index !== postdocCandidateDetails.basicInfo.other_emails.length - 1 && (
              ...
              />
              <InfoItem icon={<FiPhone />} label="Phone" value={postdocCandidateDetails.basicInfo.phone} />
              <InfoItem icon={<FiCalendar />} label="Date of Birth" value={formatDate(postdocCandidateDetails.basicInfo.dob)} />
              <InfoItem icon={<FiCalendar />} label="Enrollment Date" value={formatDate(postdocCandidateDetails.basicInfo.admission_date)} />
              <InfoItem icon={<FiCalendar />} label="Graduation Date" value={formatDate(postdocCandidateDetails.basicInfo.completion_date)} />
              <InfoItem icon={<MdOutlineBloodtype />} label="Blood Group" value={postdocCandidateDetails.basicInfo.blood_group} />
              <InfoItem icon={<FaRegIdBadge />} label="Banner ID" value={postdocCandidateDetails.basicInfo.id_number} />
              <InfoItem icon={<GiPassport />} label="Passport Number" value={postdocCandidateDetails.basicInfo.passport_number} />
            </div>
          </section>

          {/* Education Timeline */}
          {postdocCandidateDetails.education?.length > 0 && (
            <section className="bg-gray-800/50 backdrop-blur-lg p-8 rounded shadow-xl">
              ...
              <div className="space-y-6 relative before:absolute before:left-8 before:h-full before:w-0.5 before:bg-gray-600">
                {postdocCandidateDetails.education.map((edu, index) => (
                  <TimelineItem
                    key={index}
                    title={edu.degree}
                    subtitle={edu.institution}
                    year={edu.passing_year}
                    isLast={index === postdocCandidateDetails.education.length - 1}
                  />
                ))}
              </div>
            </section>
        ...
        {/* Career Timeline */}
        {postdocCandidateDetails.career?.length > 0 && (
          <section className="bg-gray-800/50 backdrop-blur-lg p-8 rounded shadow-xl">
            ...
            <div className="space-y-6 relative before:absolute before:left-8 before:h-full before:w-0.5 before:bg-gray-600">
              {postdocCandidateDetails.career.map((job, index) => (
                <TimelineItem
                  key={index}
                  title={job.position}
                  subtitle={job.organization_name}
                  year={`${job.joining_year} - ${job.leaving_year || 'Present'}`}
                  isLast={index === postdocCandidateDetails.career.length - 1}
                />
              ))}
            </div>
          </section>
...
export default withAuth(PostdocCandidateDetails, 'admin'); // Pass 'admin' as the required role