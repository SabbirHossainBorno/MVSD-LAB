// app/dashboard/member_list/postdoc_candidate_list/page.js
'use client';
...
const PostdocCandidatesList = () => {
  const [postdocCandidates, setPostdocCandidates] = useState([]);
  ...
  const [totalPostdocCandidates, setTotalPostdocCandidates] = useState(0);
  const [activePostdocCandidates, setActivePostdocCandidates] = useState(0);
  const [inactivePostdocCandidates, setInactivePostdocCandidates] = useState(0);
  const [graduatePostdocCandidates, setGraduatePostdocCandidates] = useState(0);
  const router = useRouter();

  useEffect(() => {
    const fetchPostdocCandidates = async () => {
      setLoading(true); 
      try {
        const res = await fetch(`/api/member_list/postdoc_candidate_list?page=${currentPage}&search=${searchTerm}&filter=${filter}&sortOrder=${sortOrder}`);
        const data = await res.json();
        if (res.ok) {
          setPostdocCandidates(data.postdoc_candidates || []);
          setTotalPages(data.totalPages || 1);
          setTotalPostdocCandidates(data.totalPostdocCandidates ?? 0);
          setActivePostdocCandidates(data.activePostdocCandidates ?? 0);
          setInactivePostdocCandidates(data.inactivePostdocCandidates ?? 0);
          setGraduatePostdocCandidates(data.graduatePostdocCandidates ?? 0);
        } else {
          toast.error(data.message);
        }
      } catch (error) {
        toast.error('Failed to fetch Post Doc candidates');
      } finally {
        setLoading(false);
      }
    };

    const debounceFetch = setTimeout(fetchPostdocCandidates, 300); // Debounce for 300ms

    return () => clearTimeout(debounceFetch);
  }, [currentPage, searchTerm, filter, sortOrder]);

  const handleEdit = (id) => {
    router.push(`/dashboard/member_edit/postdoc_candidate_edit/${id}`);
  };

  const handleView = (id) => {
    router.push(`/dashboard/member_details/postdoc_candidate_details/${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-blue-900 text-slate-100 p-4 sm:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="relative bg-gray-800/50 backdrop-blur-lg rounded shadow-2xl p-6 md:p-8 w-full mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 md:gap-4 max-w-2xl mx-auto">
              ...
              <div className="bg-yellow-600/20 px-3 py-2 md:px-4 md:py-2 rounded flex items-center justify-center sm:justify-start w-full">
                ...
                <span className="font-medium text-sm md:text-base whitespace-nowrap">
                  Graduate {graduatePostdocCandidates}
                </span>
              </div>
              <div className="bg-green-600/20 px-3 py-2 md:px-4 md:py-2 rounded flex items-center justify-center sm:justify-start w-full">
                ...
                <span className="font-medium text-sm md:text-base whitespace-nowrap">
                  Active {activePostdocCandidates}
                </span>
              </div>
              <div className="bg-red-600/20 px-3 py-2 md:px-4 md:py-2 rounded flex items-center justify-center sm:justify-start w-full">
                ...
                <span className="font-medium text-sm md:text-base whitespace-nowrap">
                  Inactive {inactivePostdocCandidates}
                </span>
              </div>
            </div>
          </div>
        ...
        >
          <div className="flex flex-col md:flex-row gap-4 w-full">
            <div className="relative flex-[3] min-w-[240px]">
              <input
                type="text"
                placeholder="Search Post Doc candidates..."
                value={searchTerm}
        ...
        {/* Post Doc Candidates Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {postdocCandidates.length > 0 ? (
            postdocCandidates.map((postdocCandidate) => (
              <motion.div 
                key={postdocCandidate.id}
              ...
              >
                ...
                <div className="flex items-start space-x-4">
                  <div className="relative w-20 h-20 flex-shrink-0">
                    {postdocCandidate.photo ? (
                      <Image
                        src={
                          // Handle default photo path differently
                          postdocCandidate.photo.includes('default_DP.png') 
                            ? postdocCandidate.photo
                            : `/Storage/Images/Postdoc_Candidate/${postdocCandidate.photo.split('/').pop()}`
                        }
                        alt={`${postdocCandidate.first_name} ${postdocCandidate.last_name}`}
                        width={80}
                        height={80}
                    ...
                    <div
                      className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-gray-800 ${
                        postdocCandidate.status === 'Active'
                          ? 'bg-green-500'
                          : postdocCandidate.status === 'Graduate'
                          ? 'bg-yellow-500'
                          : 'bg-red-500'
                      }`}
                    />
                  ...
                  <h2 className="text-xl font-semibold truncate">
                      {postdocCandidate.first_name} {postdocCandidate.last_name}
                    </h2>
                    <p className="text-gray-400 text-sm truncate mb-2">{postdocCandidate.email}</p>
                    ...
                    <span className="px-2 py-1.5 bg-blue-600/20 text-blue-300 text-xs rounded inline-flex items-center gap-1 border border-blue-500/10">
                      ...
                      <span className="px-2 py-1.5 bg-green-600/20 text-green-300 text-xs rounded inline-flex items-center gap-1 border border-green-500/10">
                        ...
                        <span>{format(new Date(postdocCandidate.admission_date), "d MMM yyyy")}</span>
                      </span>
                      {postdocCandidate.completion_date && (
                        <span className="px-2 py-1.5 bg-red-600/20 text-red-300 text-xs rounded inline-flex items-center gap-1 border border-red-500/10">
                          ...
                          <span>{format(new Date(postdocCandidate.completion_date), "d MMM yyyy")}</span>
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex justify-end space-x-3 mt-6">
                  <button
                    onClick={() => handleView(postdocCandidate.id)}
                  ...
                  <button
                    onClick={() => handleEdit(postdocCandidate.id)}
            ...
            <div className="col-span-full text-center py-16 bg-gray-800/50 rounded">
              <p className="text-2xl text-gray-400 mb-4">No Post Doc candidates found</p>
            </div>
          )}
        </div>
...
export default withAuth(PostdocCandidatesList, 'admin'); // Pass 'admin' as the required role